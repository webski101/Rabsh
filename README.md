...
- ⚠ `last_minute_discount` / `far_future_premium` / `day_of_week` sub-shapes were **not** covered by the smoke test — best-guess shapes live in one place (`lib/scenarios.js`); a rejected shape fails that scenario visibly, never the run.
